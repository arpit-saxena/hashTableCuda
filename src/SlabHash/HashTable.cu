...
__global__ void utilitykernel::init_table(int no_of_buckets, Address * base_slabs) {
	ResidentBlock rb;
...
__device__ HashTableOperation::HashTableOperation(Instruction * ins, HashTable * h, ResidentBlock * rb, bool is_active) : hashtable(h) {
	resident_block = rb;
	instr = ins;
