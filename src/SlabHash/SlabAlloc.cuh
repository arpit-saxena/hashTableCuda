...
#ifdef __CUDACC__
#define __block_threadIdx \
  (threadIdx.x + blockDim.x * (threadIdx.y + blockDim.y * threadIdx.z))
#define __laneID (__block_threadIdx % warpSize)
#define __local_warp_id (__block_threadIdx / warpSize)
#define __global_blockIdx \
  (blockIdx.x + gridDim.x * (blockIdx.y + gridDim.y * blockIdx.z))
#define __numwarps_in_block \
  (CEILDIV(blockDim.x * blockDim.y * blockDim.z, warpSize))
#define __global_warp_id \
  (__numwarps_in_block * __global_blockIdx + __local_warp_id)
#endif  // __CUDACC__
 ...
 * which each set bit representing that the corresponding slab has been
 * allocated
 */
struct BlockBitMap {
  uint32_t bitmap[32];
...
namespace SlabAlloc {
static const int maxSuperBlocks =
    (1 << SUPERBLOCK_BITS) -
    1;  // The last superblock will not be allocated, this
        // ensures EMPTY_ADDRESS is an invalid address
__device__ Address makeAddress(uint32_t superBlock_idx,
                               uint32_t memoryBlock_idx, uint32_t slab_idx);
...
extern __device__ SuperBlock*
    dyn_allocated_superBlocks[maxSuperBlocks - initNumSuperBlocks];

__host__ void init();
__host__ void destroy();
extern __device__ BlockBitMap
    bitmaps[maxSuperBlocks * SuperBlock::numMemoryBlocks];
...
class ResidentBlock {     // Objects of this will be on thread-local memory
  Address starting_addr;  // address of the 1st memory unit of the resident
                          // block

  uint32_t resident_bitmap_line;  // local copy of the 32-bit line of the bitmap
                                  // of the resident block belonging to the lane
