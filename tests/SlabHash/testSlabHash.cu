...
// 18))) 		printf("After writing, Warp %d, Lane %d: Slab 1 - %d,
// Slab
// 2
...
// < SlabAlloc::maxSuperBlocks * SuperBlock::numMemoryBlocks) {
// uint32_t
// Bitmap = SlabAlloc::bitmaps[i / 32].bitmap[i % 32]; 		if (Bitmap != 0)
// { 			printf("SlabAlloc::bitmaps[%d].bitmap[%d] = %x, instead
// of 0\n", i / 32, i % 32, Bitmap);
//		}
//		i += gridDim.x;
//	}
...
//!=
// 0xFFFFFFFF)) 		printf("Before writing, Warp %d, Lane %d: Slab 1
// - %x, Slab 2 - %x\n", warpID(), laneID(), data1, data2);
//
...
// MEMORYBLOCK_BITS - log2slabsPerWarp; 	SlabAlloc::init(); 	int
// numBlocks = numWarps>>5, threadsPerBlock = 1024;
...
// sum_sqr_local_rbl_changes, sizeof(float))); 	avg_local_rbl_changes /=
// (numWarps
