#ifdef WIN32
#define _CRT_SECURE_NO_WARNINGS  // stupid crap VS forcing me to use the safer
...
__host__ void CUDA::launch_kernel(Triangle* buffer[2], unsigned numTriangles[2],
  ...
  int threadsPerBlock = THREADS_PER_BLOCK,
      numBlocks = CEILDIV(totalTriangles, threadsPerBlock);
  CUDA::triangleKernel<<<numBlocks, threadsPerBlock>>>(
...
__global__ void CUDA::triangleKernel(Triangle* buffer0, Triangle* buffer1,
                                     unsigned numTriangles0,
                                     unsigned numTriangles1, Triangle* mesh0,
                                     Triangle* mesh1, HashTable* d_h,
  ...
  while (global_thread_id < maxThreadsToRun) {
    ...
    } else {  // This is an invalid thread, all the triangles have already been
              // assigned to previous threads This thread just needs to run for
              // the warp-cooperative HashTable functions
      buffer = mesh = nullptr;
      triangleindex = 0;
      meshindex = 2;
    }
    ...
    CUDA::updateTrianglePosition(currtriangle, triangleindex, meshindex, d_h,
...
void OpenGLScene::runCuda() {
  ...
  for (int i = 0; i < 2; ++i) {
    gpuErrchk(cudaGraphicsResourceGetMappedPointer(
        (void**)(dptr + i), num_bytes + i, vbo_resource[i]));
    if (num_bytes[i] != this->meshes[i].numTriangles * sizeof(Triangle)) {
      std::cerr << "VBO " << i
                << " not initialized properly, no. of bytes in VBO found after "
                   "mapping to CUDA = "
                << num_bytes[i] << " and no. of bytes of triangles in mesh = "
                << this->meshes[i].numTriangles * sizeof(Triangle) << std::endl;
...
OpenGLScene::OpenGLScene(Mesh h_meshes[2], glm::mat4 init_model_mat[2],
                         glm::mat4 trans_model_mat[2], HashTable* d_h) {
  for (int i = 0; i < 2; ++i) {
    for (int t = 0; t < h_meshes[i].numTriangles; ++t) {
      for (int v = 0; v < 3; ++v) {
        h_meshes[i].triangles[t].vertices[v].hasCollided =
            0.0f;  // All meshes will be uncollided initially
      }
    }
    meshes[i].numTriangles = h_meshes[i].numTriangles;
    gpuErrchk(cudaMalloc((void**)&meshes[i].triangles,
                         meshes[i].numTriangles * sizeof(Triangle)));
    gpuErrchk(cudaMemcpy(meshes[i].triangles, h_meshes[i].triangles,
                         meshes[i].numTriangles * sizeof(Triangle),
                         cudaMemcpyDefault));
  }
  ...
  gpuErrchk(
      cudaMalloc((void**)&(this->trans_model_mat), 2 * sizeof(glm::mat4)));
  gpuErrchk(
      cudaMalloc((void**)&(this->identity_model_mat), 2 * sizeof(glm::mat4)));
  gpuErrchk(cudaMemcpy(this->init_model_mat, init_model_mat,
                       2 * sizeof(glm::mat4), cudaMemcpyDefault));
  gpuErrchk(cudaMemcpy(this->trans_model_mat, trans_model_mat,
                       2 * sizeof(glm::mat4), cudaMemcpyDefault));
  gpuErrchk(cudaMemcpy(this->identity_model_mat, identity_model_mat,
                       2 * sizeof(glm::mat4), cudaMemcpyDefault));
...
void OpenGLScene::registerVBO() {
  for (int i = 0; i < 2; ++i) {
    gpuErrchk(cudaGraphicsGLRegisterBuffer(
        vbo_resource + i, VBO[i], cudaGraphicsRegisterFlagsWriteDiscard));
  }
}
...
void OpenGLScene::prepdraw() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                          (void*)offsetof(Vertex, hasCollided));
...
int OpenGLScene::render() {
  ...
  glm::mat4 projection = glm::perspective(glm::radians(45.0f),
                                          ::getCurrAspectRatio(), 0.1f, 100.0f);
  sh.setMatrix4fv("projection", projection);

  this->sh = &sh;

  Shader lightCubeShader("render/Shaders/lightCubeShader.vs",
                         "render/Shaders/lightCubeShader.fs");
  lightCubeShader.use();
